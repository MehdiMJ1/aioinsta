...
async def test_post_creating(client, database) -> None:
    ...
    async with client.server.app["db"] as conn:
        ...
        assert (
            ...
            and post.get("text") == "Test post"
            and post.get("image") == "Base64 image"
            and isinstance(post.get("timestamp"), datetime.datetime)
        )
    ...
    async with client.server.app["db"] as conn:
        ...
        for n, user in enumerate(users):
            for m, post in enumerate(posts):
                ...
                assert await get_posts_likes_count(conn, post_id=post) == n + 1
                ...
                assert await get_posts_likes_count(conn, post_id=post) == n
...
async def test_post_comments(client, database) -> None:
    ...
    async with client.server.app["db"] as conn:
        ...
        assert (
            comment.get("user_id") == user_id
            and comment.get("text") == "Test comment"
            and isinstance(comment.get("timestamp"), datetime.datetime)
        )
